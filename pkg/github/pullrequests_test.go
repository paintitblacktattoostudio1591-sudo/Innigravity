...
func Test_GetPullRequestComments(t *testing.T) {
	...
	// Note: get_review_comments now uses GraphQL API to fetch review threads.
	// Detailed integration testing for the GraphQL implementation should be done via e2e tests.
	// The tool schema verification above confirms the tool is properly defined.
}
...
func Test_GetPullRequestReviews(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, handler := PullRequestRead(stubGetClientFn(client), stubGetGQLClientFn(gqlClient), cache, translations.NullTranslationHelper, flags)
